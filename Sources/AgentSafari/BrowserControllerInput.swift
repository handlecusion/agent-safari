...
extension BrowserController {
    ...
    // Programmatically clicks the file input to open the WKUIDelegate file panel. The
    // pending upload URLs must already be armed for the target webView.
    private func clickUploadElement(selector: String) async throws {
        let selectorLiteral = try javaScriptStringLiteral(selector)
        let script = """
        (() => {
          const target = \(selectorLiteral);
    ...
    func upload(selector: String, paths: [String]) async throws -> [String: String] {
        ...
        // Validate the target and arm the pending URLs BEFORE the click so the file
        // panel callback has files to consume the moment WebKit raises it.
        ...
        do {
            try await clickUploadElement(selector: selector)
        } catch {
            disarmPendingUploadFileURLs(for: targetWebView)
            throw error
        }

        // The open panel is delivered asynchronously; poll until the input reports the
        // expected file count or the bounded window elapses.
        var verification = try await uploadVerification(selector: selector)
        for _ in 0..<20 where verification.count != fileURLs.count {
            try await Task.sleep(nanoseconds: 50_000_000)
            verification = try await uploadVerification(selector: selector)
        }

        // If the panel never fired, the pending URLs are still armed for this tab.
