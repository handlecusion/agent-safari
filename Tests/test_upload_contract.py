...
def test_upload_method_arms_before_click_and_verifies_count() -> None:
    ...
    # WebKit raises the open panel only for real user activation — the native
    # Quartz trigger is attempted only on the visible tab
    assert "targetWebView === activeTabWebView" in upload_body
    # Arming must precede the native click so the panel callback has files to consume
    arm_index = upload_body.index("armPendingUploadFileURLs(fileURLs, for: targetWebView)")
    click_index = upload_body.index("dispatchNativeClick(at: hitTarget)")
    assert arm_index < click_index, "must arm pending upload URLs before clicking"
    assert "clickUploadElement" not in upload_body, "synthetic JS click cannot open the panel"
    # Deterministic DataTransfer fallback when the panel cannot fire (no Accessibility
    # permission, background tab), with explicit method/fallbackUsed evidence
    assert "setUploadFilesViaDataTransfer(selector: selector, fileURLs: fileURLs)" in upload_body
    assert '"method": method' in upload_body
    assert '"fallbackUsed"' in upload_body
    assert "new DataTransfer()" in source
    assert "uploadFallbackMaxBytesPerFile" in source
    assert "uploadFileTooLargeForFallback" in source
...
def test_mcp_tools_json_pins_upload_contract() -> None:
    ...
    assert upload["result"] == ["selector", "files", "fileCount", "changeEventSynthesized", "method", "fallbackUsed", "strategy", "tabId"]
