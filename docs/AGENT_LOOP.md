...
- If click/fill fails on actionability, branch on JSON-RPC `error.code` before retrying. Stable codes include `actionability_stale_ref`, `actionability_refs_unavailable`, `actionability_missing_selector`, `actionability_disabled`, `actionability_hidden`, `actionability_off_viewport`, and `actionability_occluded`. Other stable codes: `wait_timeout`, `invalid_url`, `missing_param`, `invalid_param`, `unknown_method`, `element_resolution_failed`, `screenshot_failed`, `page_measurement_failed`, `javascript_encoding_failed`, `socket_error`, `unknown_tab`.
