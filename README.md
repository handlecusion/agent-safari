...
/tmp/agent-safari-v0.0.6-macOS-ARM64/install.sh
```
...
- `click <selector> --native` may fall back to DOM click if the native event cannot be verified. That returns `method: "dom-fallback"`, `nativeVerified: false`, `fallbackUsed: true`, plus `nativeError` and `nativeErrorCode`.
