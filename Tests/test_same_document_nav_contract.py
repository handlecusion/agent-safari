...
def test_same_document_detection_helper_exists() -> None:
    ...
    assert "func isSameDocumentNavigation(to target: URL, in webView: WKWebView) -> Bool" in source
...
def test_navigate_branches_on_same_document_before_load() -> None:
    ...
    branch_idx = func_body.find("if isSameDocumentNavigation(to: url, in: target)")
...
def test_same_document_branch_uses_javascript_not_continuation() -> None:
    source = read(NAVIGATION)
    func_body = source.split("func navigate(")[1].split("\n    }")[0]
    branch = func_body.split("if isSameDocumentNavigation(to: url, in: target)")[1].split("withCheckedThrowingContinuation")[0]
